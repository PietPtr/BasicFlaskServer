...
def add_cors_header(response):
    ...
    if 'HTTP_ORIGIN' in request.environ and ("localhost" in request.environ['HTTP_ORIGIN'] or "penny.vck.tv:4000" in request.environ['HTTP_ORIGIN']):
        response.headers['Access-Control-Allow-Origin'] = request.environ['HTTP_ORIGIN']
    else:
        response.headers['Access-Control-Allow-Origin'] = 'https://my.vck.tv'
    return response
